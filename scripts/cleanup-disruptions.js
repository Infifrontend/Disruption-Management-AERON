...
async function deleteFlightDisruption(disruptionId) {
  ...
  try {
    ...
    const relatedTables = ["crew_disruption_mapping", "hotel_bookings"];
    ...
    try {
      ...
      console.log(
        `Deleted ${result.rowCount} records from pending_recovery_solutions`,
      );
    } catch (error) {
    ...
    console.log(
      `Successfully deleted disruption ${disruptionId} and all related data`,
    );
...
async function bulkDeleteDisruptions(criteria) {
  ...
  try {
    ...
    console.log(
      `Found ${disruptionIds.length} disruptions to delete:`,
    ...
    await client.query("COMMIT");
    console.log(
      `Successfully deleted ${mainResult.rowCount} disruptions and all related data`,
    );
