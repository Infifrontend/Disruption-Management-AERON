      ...
      {preservedSelections && activeContext?.fromExecution && (
        <Card className="bg-blue-50 border-blue-200">
          ...
          <CardContent className="space-y-4">
            ...
            {/* Rotation Plan Info */}
            {recoveryOption?.rotation_plan && (
              <Card className="bg-slate-50 border-slate-200">
                <CardHeader className="pb-3">
                  <CardTitle className="flex items-center gap-2 text-slate-800 text-lg">
                    <Plane className="h-5 w-5" />
                    Rotation Plan Impact
                  </CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  {/* Impacted Flights */}
                  {recoveryOption.rotation_plan.nextSectors && 
                   Array.isArray(recoveryOption.rotation_plan.nextSectors) && 
                   recoveryOption.rotation_plan.nextSectors.length > 0 && (
                    <div className="p-3 bg-white rounded border border-slate-200">
                      <h5 className="font-medium text-slate-900 mb-2 flex items-center gap-2">
                        <Users className="h-4 w-4" />
                        Impacted Flights ({recoveryOption.rotation_plan.nextSectors.length})
                      </h5>
                      <div className="space-y-2">
                        {recoveryOption.rotation_plan.nextSectors.slice(0, 3).map((sector, index) => (
                          <div key={index} className="flex items-center justify-between p-2 bg-gray-50 rounded text-xs">
                            <div className="flex items-center gap-2">
                              <Badge className="bg-slate-100 text-slate-700">
                                {getSafeStringValue(sector.flight || sector.flightNumber || `Flight ${index + 1}`)}
                              </Badge>
                              <span className="text-slate-600">
                                {getSafeStringValue(sector.departure || sector.departureTime || "TBD")}
                              </span>
                            </div>
                            <div className="flex items-center gap-1">
                              {sector.delay && (
                                <Badge className="bg-orange-100 text-orange-700">
                                  Delay: {getSafeStringValue(sector.delay)}
                                </Badge>
                              )}
                              <span className="text-slate-600">
                                {getSafeStringValue(sector.passengers || "0")} pax
                              </span>
                            </div>
                          </div>
                        ))}
                        {recoveryOption.rotation_plan.nextSectors.length > 3 && (
                          <div className="text-center text-xs text-slate-500">
                            +{recoveryOption.rotation_plan.nextSectors.length - 3} more flights affected
                          </div>
                        )}
                      </div>
                    </div>
                  ...
                  {/* Cost Impact */}
                  {recoveryOption.rotation_plan.costBreakdown && (
                    <div className="p-3 bg-white rounded border border-slate-200">
                      <h5 className="font-medium text-slate-900 mb-2 flex items-center gap-2">
                      ...
                      <div className="space-y-1 text-xs">
                        {Object.entries(recoveryOption.rotation_plan.costBreakdown).map(([key, value]) => (
                  ...
                  {recoveryOption.rotation_plan.operationalConstraints && (
                    <div className="p-3 bg-white rounded border border-slate-200">
                      <h5 className="font-medium text-slate-900 mb-2 flex items-center gap-2">
                      ...
                      <div className="space-y-1 text-xs">
                        {Object.entries(recoveryOption.rotation_plan.operationalConstraints).map(([key, value]) => (
                          <div key={key} className="flex justify-between">
                            <span className="text-slate-600 capitalize">
                              {key.replace(/([A-Z])/g, ' $1').trim()}:
                            </span>
