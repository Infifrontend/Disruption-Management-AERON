...
const transformFlightData = (disruption: FlightDisruption) => {
  ...
  const cleanDestinationCity =
    ...
    disruption.destinationCity !== "unknown"
      ? dis<ctrl62>    disruptionCategories.length > 0
  ...
  const filteredFlights = flights.filter((flight) => {
    ...
    if (filters.hub !== "all") {
      ...
      const isRelatedToHub =
        flight.origin === filters.hub || flight.destination === filters.hub;
  ...
  const handleGenerateRecoveryOptions = async (flight) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/recovery-options/generate/${flightId}`,
        {
          method: "POST",
          headers: {
      ...
      setLoadingRecovery((prev) => ({
        ...prev,
        [flight.id || flight.flightNumber]: false,
      }));
    }
  };
      ...
      if (options.length === 0) {
        ...
        if (generateResponse.ok) {
          ...
          if (generateResult.success) {
            ...
            const newOptionsResponse = await fetch(
              `/api/recovery-options/${flightId}`,
            );
  ...
  const handleAddDisruption = async () => {
    ...
    const newFlightData = {
      ...
      categorization:
        selectedCategory?.category_name || newDisruption.categorization,
  ...
  return (
    <div className="space-y-6 pb-32">
                          ...
                          <Table className="min-w-full">
                            ...
                            <TableBody>
                              {paginatedInboundFlights.map((flight) => (
                                <TableRow
                                  key={flight.id}
                                  className={`hover:bg-accent hover:text-accent-foreground transition-colors cursor-pointer ${selectedFlight?.id === flight.id ? "bg-muted dark:bg-muted/50 border-muted dark:border-muted/50" : ""}`}
                                ...
                                >
                                  ...
                                  <TableCell>
                                    <div className="flex items-center gap-2 max-w-[140px]">
                                      ...
                                      <span className="font-medium truncate text-blue-600 dark:text-blue-400">
                                        {flight.destination}
                                      </span>
                                    </div>
                                    <div
                          ...
                          filteredFlights.length > 0 && (
                            <div className="text-center py-12">
                              ...
                              <p className="text-gray-500 mb-4">
                                No inbound flights to the selected hub(s) match
                                  ...
                                  <TableCell>
                                    ...
                                    >
                                      {loadingRecovery[
                                        ...
                                        <>
                                          <div className="animate-spin rounded-full h-3 w-3 border-2 border-blue-600 dark:border-blue-400 border-t-transparent mr-1"></div>
                                          Loading...
                                        </>
                                      ) : (
                                        <>
                        ...
                        {outboundFlights.length === 0 &&
                          filteredFlights.length > 0 && (
                            <div className="text-center py-12">
                              ...
                              <p className="text-gray-500 mb-4">
                                No outbound flights from the selected hub(s)
                                match your current filter criteria.
                              </p>
                            </div>
                          )}
                      </>
      ...
      {selectedFlight && (
        <div className="sticky bottom-0 left-0 right-0 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 p-4 shadow-lg z-10 mt-4">
