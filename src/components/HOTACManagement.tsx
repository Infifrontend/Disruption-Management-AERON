...
export function HOTACManagement() {
  ...
  useEffect(() => {
    ...
    if (baseAirportFilter !== "all") {
      filtered = filtered.filter(
        (record) => record.baseAirport === baseAirportFilter,
      );
    }
    ...
    if (pairingTypeFilter !== "all") {
      filtered = filtered.filter(
        (record) =>
          record.pairingDetails.pairingType.toLowerCase() ===
          pairingTypeFilter.toLowerCase(),
      );
    }
  ...
  const handleRebookHotel = (record) => {
    alert(
      `Initiating hotel rebooking for ${record.flightNumber} - ${record.hotelName}`,
    );
  ...
  const handleChangeStatus = (record, newStatus) => {
    alert(
      `Changing status of ${record.confirmationNumber} from ${record.bookingStatus} to ${newStatus}`,
    );
  ...
  const handleNotifyHotel = (record) => {
    alert(
      `Sending notification to ${record.hotelName} about booking ${record.confirmationNumber}`,
    );
  ...
  const handleNotifyTransport = (record) => {
    alert(
      `Sending notification to ${record.transportProviderName} for pickup at ${record.pickupTime}`,
    );
  ...
  const handleNotifyCrew = (record) => {
    alert(
      `Sending accommodation details to crew members: ${record.crewIds.join(", ")}`,
    );
  ...
  const handleGenerateVoucher = (record) => {
    alert(
      `Generating accommodation voucher for ${record.flightNumber} - ${record.confirmationNumber}`,
    );
  ...
  const handleCancelBooking = (record) => {
    if (
      confirm(
        `Are you sure you want to cancel booking ${record.confirmationNumber}?`,
      )
    ) {
      alert(
        `Cancelling booking for ${record.flightNumber} at ${record.hotelName}`,
      );
  ...
  const handleEscalateIssue = (record) => {
    alert(
      `Escalating accommodation issue for ${record.flightNumber} to management`,
    );
  ...
  const handleRequestPriority = (record) => {
    alert(
      `Requesting priority handling for ${record.flightNumber} accommodation`,
    );
  ...
  const getPairingTypeBadge = (type) => {
    switch (type.toLowerCase()) {
      ...
      case "domestic":
        return (
          <Badge className="bg-flydubai-navy text-white">
      ...
      case "regional":
        return (
          <Badge className="bg-flydubai-orange text-white">
  ...
  const getComplianceBadge = (compliance, maxDutyExceeded, restMet) => {
    if (compliance === "N/A") {
      return (
        <Badge className="bg-gray-100 text-gray-700 border-gray-200">N/A</Badge>
      );
    }
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-semibold text-flydubai-navy">
            Crew HOTAC & Pairing Management
          </h2>
          <p className="text-muted-foreground">
            Hotel, Transport Accommodation & Crew Pairing Records
          </p>
        </div>
          ...
          <div className="text-sm">
            <div className="flex items-center gap-4">
              <div className="text-center">
                <p className="font-medium text-flydubai-blue">
                  {hotacRecords.length}
                </p>
              ...
              <div className="text-center">
                <p className="font-medium text-green-600">
                  {
                    hotacRecords.filter((r) => r.bookingStatus === "Confirmed")
                      .length
                  }
                </p>
              ...
              <div className="text-center">
                <p className="font-medium text-yellow-600">
                  {
                    hotacRecords.filter((r) => r.bookingStatus === "Pending")
                      .length
                  }
                </p>
              ...
              <div className="text-center">
                <p className="font-medium text-red-600">
                  {hotacRecords.filter((r) => r.priority === "Urgent").length}
                </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-6 gap-4 mb-4">
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={setBaseAirportFilter}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={setPairingTypeFilter}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                  ...
                  <TableRow className="hover:bg-blue-50">
                    ...
                    <TableCell>
                      <div className="space-y-1">
                        ...
                        <Badge
                          variant="outline"
                          className="border-flydubai-orange text-flydubai-orange"
                        >
                          {record.baseAirport}
                        </Badge>
                      ...
                      <div className="text-sm space-y-1">
                        ...
                        <div className="flex items-center gap-1 text-muted-foreground">
                          ...
                          <span className="text-xs">
                            {record.pairingDetails.numberOfFlights} flights
                          </span>
                        </div>
                        {getPairingStatusBadge(
                          record.pairingDetails.pairingStatus,
                        )}
                      </div>
                    </TableCell>
                        ...
                        <Dialog>
                          ...
                          <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
                            <DialogHeader>
                              ...
                              <DialogDescription>
                                View comprehensive details for{" "}
                                {record.flightNumber} including pairing
                                information, hotel and transport arrangements.
                              </DialogDescription>
                            </DialogHeader>
                            ...
                            <div className="space-y-1">
                              ...
                              {record.bookingStatus === "Pending" && (
                                <Button
                                  ...
                                  variant="ghost"
                                  onClick={() =>
                                    handleChangeStatus(record, "Confirmed")
                                  }
                                  className="w-full justify-start h-8 px-2 hover:bg-green-50 hover:text-green-700"
                                >
...
function ExpandedRowDetails({ record }) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
      ...
      <div>
        <h4 className="font-medium text-flydubai-navy mb-3">
          Duty & Rest Information
        </h4>
        <div className="space-y-3">
          <div className="p-3 bg-white rounded border">
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span
                  className={`font-medium ${record.pairingDetails.maxDutyExceeded ? "text-red-600" : ""}`}
                >
                  {record.pairingDetails.dutyHours}
                  {record.pairingDetails.maxDutyExceeded && (
              ...
              <div className="flex justify-between">
                ...
                <span>
                  {record.pairingDetails.homeBaseReturn ? "Yes" : "No"}
                </span>
              </div>
            </div>
          </div>
      ...
      <div>
        ...
        <div className="space-y-3">
          <div className="p-3 bg-white rounded border">
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <a
                  href={`tel:${record.driverContactNumber}`}
                  className="text-flydubai-blue hover:underline"
                >
                  {record.driverContactNumber}
                </a>
              </div>
          ...
          <div className="flex gap-2">
            <Button
              size="sm"
              className="bg-flydubai-blue hover:bg-flydubai-blue/90 text-white flex-1"
            >
            ...
            <Button
              size="sm"
              className="bg-flydubai-orange hover:bg-flydubai-orange/90 text-white flex-1"
            >
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-blue-50 p-4 rounded-lg">
        <h4 className="font-medium mb-3 text-flydubai-navy">
          Pairing Overview
        </h4>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="text-center">
            <div className="text-2xl font-bold text-flydubai-navy">
              {record.pairingDetails.layoverCount}
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
          <h4 className="font-medium mb-4 text-flydubai-navy">
            Flight & Pairing Information
          </h4>
            ...
            {(record.pairingDetails.maxDutyExceeded ||
              ...
              <div className="p-3 bg-red-50 border border-red-200 rounded-lg">
                ...
                <ul className="text-sm text-red-600 space-y-1">
                  {record.pairingDetails.maxDutyExceeded && (
                    <li>• Maximum duty time exceeded</li>
                  )}
                  {!record.pairingDetails.restRequirementsMet && (
                    <li>• Minimum rest requirements not met</li>
                  )}
                </ul>
              </div>
            )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-3 text-sm">
            ...
            <div>
              ...
              <div className="font-semibold text-flydubai-orange text-lg">
                {record.totalCost}
              </div>
            </div>
            <div>
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="text-center p-3 bg-white rounded border">
              <div className="text-lg font-semibold text-flydubai-blue">
                {record.pairingDetails.creditHours}
              </div>
              <div className="text-xs text-muted-foreground">Credit Hours</div>
            </div>
            <div className="text-center p-3 bg-white rounded border">
              <div className="text-lg font-semibold text-flydubai-orange">
                {record.totalCost}
              </div>
