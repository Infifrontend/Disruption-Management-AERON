...
// const { airlineConfig } = useAirlineTheme(); // Removed to avoid uncaught TypeError
...
export const getPNRGroupTemplates = (airlineConfig) => { // Added airlineConfig as a parameter
  return [
...
export const getIndividualPNRTemplates = (airlineConfig) => { // Added airlineConfig as a parameter
  return [
    {
...
export const generateAffectedPassengers = (flight, option) => {
  const { airlineConfig } = useAirlineTheme(); // Moved hook call inside the function
  const totalPassengers = flight?.passengers || 167
  const passengers = []

  // Get PNR group templates
  const pnrGroups = getPNRGroupTemplates(airlineConfig) // Pass airlineConfig
  const individualPNRTemplates = getIndividualPNRTemplates(airlineConfig); // Pass airlineConfig
  ...
  // Add individual passengers if there are still slots to fill and we have individual templates
  const remainingPassengers = totalPassengers - passengers.length;
  if (remainingPassengers > 0) {
    ...
    for (let i = 0; i < remainingPassengers && i < individualTemplates.length; i++) {
      ...
      const contactEmail = passengerTemplate.contactEmail || `${passengerTemplate.name.toLowerCase().replace(/\s+/g, '.')}@email.com`;
      const contactPhone = passengerTemplate.contactPhone || '+971 50 000 0000';

      let status = 'Rebooking Required';
      if (requiresPassengerReaccommodation(option)) {
        if (template.priority === 'VIP') {
          status = Math.random() > 0.7 ? 'Rebooking Required' : 'Accommodation Needed';
        } else if (template.priority === 'Premium') {
          status = Math.random() > 0.5 ? 'Rebooking Required' : 'Alternative Flight';
        } else {
          status = Math.random() > 0.3 ? 'Rebooking Required' : 'Accommodation Needed';
        }
      ...
      const passenger = {
        ...
        preferences: {
          seatPreference: passengerTemplate.seatPreference || 'Any',
          mealPreference: passengerTemplate.mealPreference || 'Standard',
          classPreference: template.priority === 'VIP' ? 'Business' :
                         template.priority === 'Premium' ? 'Premium Economy' : 'Economy',
          loyaltyTier: passengerTemplate.loyaltyTier
        },
        connectedFlights: Math.random() > 0.7 ? ['FZ567', 'FZ892'] : [],
