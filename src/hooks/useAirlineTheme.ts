...
import {
  ...
  injectAirlineTheme,
  type AirlineTheme,
} from "../config/airlineConfig";

export function useAirlineTheme() {
  const [airlineConfig, setAirlineConfig] =
    useState<AirlineTheme>(getAirlineConfig());
  ...
  const updateAirlineTheme = (airlineCode: string) => {
    ...
    console.warn(
      "Dynamic airline switching requires environment variable update and page reload",
    );
  };
